...
```bash
device.x$ sam logs -n StockCheckerFunction --stack-name test.x --tail
```
...
```bash
device.x$ cd functions/update-status
update-status$ npm install
update-status$ npm run test
```
...
```bash
sam delete --stack-name device.x
```
